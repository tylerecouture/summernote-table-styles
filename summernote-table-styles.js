    ...
    $.extend($.summernote.plugins, {
        ...
        'tableStyles': function (context) {
            ...
            self.updateStyles = function($node, chosenItem, exclusiveStyles) {
                ...
                else { // Inclusive classes: multiple are ok
            ...
            self.getListItem = function(value, label, isExclusive, isDefault=false) {
                var item =  '<li><a href="#" class="' + (isExclusive ? 'exclusive-item' : 'inclusive-item')
                + (isDefault ? ' checked' : '') + '" '
                + 'style="display: block;" data-value="' + value + '">'
                + '<i class="note-icon-menu-check" ' + (!isExclusive ? 'style="color:#00ffc0;" ' : '') + '></i>'
                + ' ' + label + '</a></li>'
                return item;
            }
        }
